...
?>';
	...
	function makeSubset($file, &$subset, $TTCfontID=0, $debug=false, $useOTL=false) {	// mPDF 5.7.1
		...
		ksort($subsetglyphs);
		$n = 0;
		$fsLastCpengajuanndex = 0;	// maximum Unicode index (character code) in this font, according to the cmap subtable for platform ID 3 and platform- specific encoding ID 0 or 1.
		foreach($subsetglyphs AS $originalGlyphIdx => $uni) {
			$fsLastCpengajuanndex = max($fsLastCpengajuanndex , $uni); 
		...
		if (isset($this->tables['OS/2'])) { 
			...
			if (_RECALC_PROFILE) {
				...
				$os2 = $this->_set_ushort($os2, 66, $fsLastCpengajuanndex);
...
//================================================================================
	...
	function makeSubsetSIP($file, &$subset, $TTCfontID=0, $debug=false, $useOTL=0) {	// mPDF 5.7.1
		...
		if (isset($this->tables['OS/2'])) { 
			...
			$os2 = $this->_set_ushort($os2, 64, 0x01);		// FirstCpengajuanndex
			$os2 = $this->_set_ushort($os2, 66, count($subset));		// LastCpengajuanndex
